...
class Application():
    ...
    def chama_analisador(self): 
        ...
        for tok in lexer:
            ...
            if tok.type in ('texto_errado',
                ...
                add_lista_saida(
                    tok, c, f"Tamanho da variavel maior que o permitido")
            else:
        ...
        if len(pilha_parenteses) > 0:
            ...
            self.saida.insert('', tk.END, values=[
                              "Parenteses aberto na linha " + pilha_parenteses.pop() + " não foi fechado."])
        if len(pilha_chaves) > 0:
            erros += 1
            self.saida.insert('', tk.END, values=[
                              "Chaves aberto na linha " + pilha_chaves.pop() + " não foi fechada."])
        if len(pilha_colchete) > 0:
            erros += 1
            self.saida.insert('', tk.END, values=[
                              "Colchetes aberto na linha " + pilha_colchete.pop() + " não foi fechado."])
        ...
        self.scrollAnalise = ttk.Scrollbar(
            self.frame_2, orient='vertical', command=self.saida.yview)
        self.scrollAnalise.place(
            relx=0.979, rely=0.0192, relwidth=0.02, relheight=0.92)
        self.saida['yscrollcommand'] = self.scrollAnalise

    def botoes(self):
        # botao limpar
        self.bt_limpar = Button(text="Limpar", bd=2, bg="#FF6347", font=(
            '', 11), command=self.limpa_telaentrada)
        self.bt_limpar.place(relx=0.74, rely=0.92,
                             relwidth=0.1, relheight=0.05)

        # botao executar
        self.bt_executar = Button(text="Executar", bd=2, bg="lightgreen", font=(
            '', 11), command=self.chama_analisador)
        self.bt_executar.place(relx=0.85, rely=0.92,
                               relwidth=0.1, relheight=0.05)

        # criação da label e entrada do código
        self.lb_codigo = Label(text="Codigo Fonte", bg="white", font=('', 12))
        self.lb_codigo.place(relx=0.001, rely=-0.001,
                             relwidth=0.2, relheight=0.07)

        # criação da label da analise lexica
        self.lb_analise = Label(text="Saida", bg="white", font=('', 12))
        self.lb_analise.place(relx=0.005, rely=0.62,
                              relwidth=0.28, relheight=0.07)

        self.codigo_entry = tk.Text(self.frame_1)
        self.codigo_entry.place(relx=0.001, rely=0.001,
                                relwidth=0.995, relheight=0.995)

        self.scroll_bar = ttk.Scrollbar(
            self.frame_1, orient='vertical', command=self.codigo_entry.yview)
        self.scroll_bar.place(relx=0.982, rely=0.0019,
                              relwidth=0.015, relheight=0.99)
    ...
    def Menus(self):
        ...
        def tokens():
            ...
            # Criando tabela
            newWindow = ttk.Treeview(newWindow, height=3, column=(
                'col1', 'col2', 'col3', 'col4'))
            # Nomeando cabeçalhos
            ...
            newWindow.place(relx=0.001, rely=0.01,
                            relwidth=0.999, relheight=0.95)

            newWindow.insert("", 1, text="", values=(
                "ifsuldeminas", "ifsuldeminas", "ifsuldeminas", "Palavra Reservada ifsuldeminas"))
            newWindow.insert("", 3, text="", values=(
                "INICIO", "INICIO", "INICIO", "Palavra Reservada INICIO"))
            newWindow.insert("", 4, text="", values=(
                "COMPILADORES", "COMPILADORES", "COMPILADORES", "Palavra Reservada COMPILADORES"))
            newWindow.insert("", 5, text="", values=(
                "FIM", "FIM", "FIM", "Palavra Reservada FIM"))
            newWindow.insert("", 7, text="", values=(
                "LER", "LER", "LER", "Palavra Reservada LER"))
            newWindow.insert("", 8, text="", values=(
                "IMPRIMIR", "IMPRIMIR", "IMPRIMIR", "Palavra Reservada IMPRIMIR"))
            newWindow.insert("", 9, text="", values=(
            ...
            newWindow.insert("", 12, text="", values=(
                "SENAO_SE", "SENAO_SE", "SENAO_SE", "Palavra Reservada SENAO_SE"))
            newWindow.insert("", 13, text="", values=(
                "PARA", "PARA", "PARA", "Palavra Reservada PARA"))
            newWindow.insert("", 14, text="", values=(
                "ENQUANTO", "ENQUANTO", "ENQUANTO", "Palavra Reservada ENQUANTO"))
            newWindow.insert("", 15, text="", values=(
                "FACA", "FACA", "FACA", "Palavra Reservada FACA"))
            newWindow.insert("", 16, text="", values=(
            ...
            newWindow.insert("", 28, text="", values=(
                "fecha_parenteses", ")", ")", "Operador de Prioridade fecha parenteses"))
            newWindow.insert("", 29, text="", values=(
                "abre_chaves", "{", "{", "Operador de Prioridade abre chaves"))
            newWindow.insert("", 30, text="", values=(
                "fecha_chaves", "}", "}", "Operador de Prioridade fecha chaves"))
            newWindow.insert("", 31, text="", values=(
            ...
            newWindow.insert("", 42, text="", values=(
            ...
            newWindow.insert("", 47, text="", values=(
                "ponto_virgula", ";", ";", "Operador de Execução ponto e vírgula"))
            newWindow.insert("", 48, text="", values=(
